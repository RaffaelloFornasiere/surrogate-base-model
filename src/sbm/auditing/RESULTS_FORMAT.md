...
The JSON Schema is [manifest.schema.json](manifest.schema.json). Manifest and
...
The 2026-09-17 backfill verified 28 published bundles (25 existing HF reports
plus Patchscopes, SelfIE, and Steering uploaded from local exports). All report
content hashes were preserved. Five branches contained no report and were left
untouched. Canonical URLs were retained, and the old local source IDs are aliases
of the published bundles. The webapp discovers the full set with both the local
catalog and local-report registry disabled.

The [migration record](../../../docs/results-manifest-migration-2026-09-17.json)
contains the verified commit SHAs, report hashes, sizes, canonical paths, and
branches without reports. The backfill utility is
`scripts/maintenance/migrate_result_manifests.py`; it validates a plan before
`--apply`, uses guarded metadata commits, and verifies report content afterward.
